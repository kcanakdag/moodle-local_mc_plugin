...
### Changed
- **Profile Image Sizes**: Event payloads now include all three Moodle profile image sizes instead of just the 100x100 icon
  - `profileimageurl_512x512` (f3) — largest available
  - `profileimageurl_100x100` (f1) — standard icon
  - `profileimageurl_35x35` (f2) — small thumbnail
  - **Breaking**: Replaces the previous single `profileimageurl` field
...
### Added
- **Profile Image URL**: User profile image URL is now included in event payloads (`profileimageurl` field)
