...
/**
   ...
   */
const postForm=async(url,data)=>{const params=new URLSearchParams;Object.entries(data).forEach((_ref=>{let[key,value]=_ref;params.append(key,value)}));return(await fetch(url,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:params.toString()})).json()};_exports.initConnection=async(connectUrl,sesskey)=>postForm(connectUrl,{action:"init",sesskey:sesskey});_exports.pollConnectionStatus=async(apiUrl,token)=>(await fetch("".concat(apiUrl,"/connect/status?token=").concat(encodeURIComponent(token)))).json();_exports.saveCredentials=async(saveUrl,sesskey,siteKey,siteSecret)=>postForm(saveUrl,{action:"save",sesskey:sesskey,site_key:siteKey,site_secret:siteSecret});_exports.getConnectionStatus=async(syncUrl,sesskey)=>postForm(syncUrl,{action:"status",sesskey:sesskey});_exports.saveSettings=async(ajaxSaveUrl,sesskey,values)=>postForm(ajaxSaveUrl,{action:"save",sesskey:sesskey,...values});_exports.syncEvents=async(syncUrl,sesskey)=>postForm(syncUrl,{action:"sync",sesskey:sesskey})}));
